...
def iothub_client_init():
    ...
    client.set_option("product_info", "Parkeerplaatsmeter")
...
def iothub_client_app_run():
    try:
        ...
        if not config.SIMULATED_DATA:
            sensor = PPM(conf_limit=0.3)
        else:
            sensor = PPMSimulator(conf_limit=0.3)
